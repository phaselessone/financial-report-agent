...
def _fallback_terms(normalized: str) -> list[str]:
    terms: list[str] = []
    for token in TOKEN_SPLIT_RE.findall(normalized):
        stripped = token.strip().lower()
        if not stripped:
            continue
        terms.append(stripped)
        if not _is_cjk_token(stripped) or len(stripped) <= 4:
            continue
        for gram_size in (2, 3):
            if len(stripped) < gram_size:
                continue
            for index in range(len(stripped) - gram_size + 1):
                terms.append(stripped[index:index + gram_size])
    return terms
...
def extract_terms(text: str, *, top_k: int = 8, min_length: int = 2) -> list[str]:
    ...
    try:
        ...
        tokens.extend(_fallback_terms(normalized))
