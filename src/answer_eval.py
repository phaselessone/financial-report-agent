...
def materialize_answer_eval_sets(
    *,
    ...
    split_specs: list[tuple[str, Path, Path]] = [("dev", dev_seed_path, dev_output_path)]
    if full_seed_path is not None and full_seed_path.exists():
    ...
    for split_name, seed_path, output_path in split_specs:
        ...
        materialized_sets[split_name] = resolved_rows
        ...
        report_splits[split_name] = {
            ...
            "industry_distribution": dict(sorted(Counter(row.get("industry", "other") for row in resolved_rows).items())),
            "intent_distribution": dict(sorted(Counter(row.get("intent", row.get("question_type", "fact")) for row in resolved_rows).items())),
    ...
    *,
    ...
    for eval_row in eval_rows:
        ...
        if result.get("failed"):
            merged = _merge_eval_row(eval_row, result, answer_hit=False, citation_hit=False, unsupported=False)
        ...
        citation_hit = compute_citation_hit(eval_row, result)
        answer_hit = compute_answer_hit(eval_row, result, chunk_lookup, citation_hit=citation_hit)
        unsupported = bool(not result.get("abstained", False) and not result.get("support_validation", {}).get("supported", False))
        merged = _merge_eval_row(eval_row, result, answer_hit=answer_hit, citation_hit=citation_hit, unsupported=unsupported)
        if eval_row["must_abstain"]:
            abstain_rows.append(merged)
        ...
        answerable_rows.append(merged)
        if not answer_hit or not citation_hit or unsupported:
            answer_badcases.append(merged)

    summary = build_answer_eval_summary(answerable_rows=answerable_rows, abstain_rows=abstain_rows)
...
def compute_citation_hit(eval_row: dict[str, Any], result_row: dict[str, Any]) -> bool:
    ...
    if intent == "report_lookup":
        ...
        return title_match(normalize_text(result_row.get("final_answer", "")), target_titles) and any(
            citation.get("doc_id") in gold_doc_ids for citation in citations
        )
    if eval_row["question_type"] in {"comparison", "inductive"}:
        matched_docs = {
            citation["doc_id"]
            for citation in citations
            if citation["doc_id"] in gold_doc_ids or citation["chunk_id"] in gold_chunk_ids
        }
        required = 2 if eval_row["question_type"] == "comparison" else min(2, max(len(gold_doc_ids), 1))
        return len(matched_docs) >= required

    for citation in citations:
        page_overlap = any(page in set(eval_row.get("gold_page_nums", [])) for page in range(int(citation["page_start"]), int(citation["page_end"]) + 1))
        if citation["chunk_id"] in gold_chunk_ids:
            return True
        if citation["doc_id"] in gold_doc_ids and page_overlap:
            return True
    return False
...
def _semantic_fact_hit(*, gold_text: str, predicted_text: str, citation_hit: bool) -> bool:
    if not citation_hit:
        return False
    return _term_overlap_ratio(gold_text, predicted_text, left_top_k=12, right_top_k=12) >= 0.25


def _value_fact_hit(*, gold_text: str, predicted_text: str, citation_hit: bool, citation_support_text: str) -> bool:
    if not citation_hit:
        return False
    ...
    citation_numeric_tokens = {normalize_for_match(token) for token in extract_numeric_tokens(citation_support_text) if normalize_for_match(token)}
    if predicted_numeric_tokens and (predicted_numeric_tokens & gold_numeric_tokens or predicted_numeric_tokens & citation_numeric_tokens):
        return True
    if not predicted_numeric_tokens:
        return _term_overlap_ratio(gold_text, predicted_text, left_top_k=12, right_top_k=12) >= 0.25
    return _term_overlap_ratio(f"{gold_text}\n{citation_support_text}", predicted_text, left_top_k=12, right_top_k=12) >= 0.35
...
def compute_answer_hit(
    ...
    *,
    ...
    if intent == "report_lookup":
        target_titles = eval_row.get("target_titles") or [eval_row.get("gold_answer", "")]
        return citation_hit and title_match(combined_prediction, target_titles)
    ...
    if intent == "numeric_fact":
        ...
        if fact_subtype == "value_fact":
            return _value_fact_hit(
                ...
                citation_hit=citation_hit,
                citation_support_text=_citation_support_text(eval_row, result_row, chunk_lookup),
            )
    ...
    return citation_hit and overlap_ratio >= threshold


def build_answer_eval_summary(*, answerable_rows: list[dict[str, Any]], abstain_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    answer_hit_rate = sum(1 for row in answerable_rows if row["answer_hit"]) / answer_denominator
    citation_hit_rate = sum(1 for row in answerable_rows if row["citation_hit"]) / answer_denominator
    ...
    return {
        ...
        "Answer Hit Rate": round(answer_hit_rate, 4),
        "Citation Hit Rate": round(citation_hit_rate, 4),
...
def build_badcase_breakdown(answer_badcases: list[dict[str, Any]]) -> list[dict[str, Any]]:
    counter: Counter[tuple[str, str, str, bool, bool, str]] = Counter(
        (
    ...
    return [
        ...
        for (intent, fact_subtype, abstain_reason, doc_guard_triggered, failed, error_type), count in sorted(counter.items(), key=lambda item: (-item[1], item[0]))
    ]
